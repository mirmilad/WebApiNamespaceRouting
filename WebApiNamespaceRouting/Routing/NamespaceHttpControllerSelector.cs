...
namespace WebApiNamespaceRouting.Routing
{
    public class NamespaceHttpControllerSelector : DefaultHttpControllerSelector
    {
        ...
        private readonly Lazy<ConcurrentDictionary<string, HttpControllerDescriptor>> _controllerTypes;
        ...
        public NamespaceHttpControllerSelector(HttpConfiguration configuration) : base(configuration)
        {
            ...
            _controllerTypes = new Lazy<ConcurrentDictionary<string, HttpControllerDescriptor>>(GetControllerTypes);
        ...
        public override HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
            ...
            foreach (var ns in namespaces)
            {
                ...
                if (ns.EndsWith("*"))
                {
                    key = _duplicateControllerTypes.Value.Keys.FirstOrDefault(k => k.StartsWith(string.Format("{0}-{1}", controllerNameAsString, ns.Substring(0, ns.Length - 1)), StringComparison.OrdinalIgnoreCase));
                }
                else
                {
                    key = string.Format("{0}-{1}", controllerNameAsString, ns);
                }
                if (!string.IsNullOrWhiteSpace(key) && _duplicateControllerTypes.Value.TryGetValue(key, out descriptor))
                {
                    return descriptor;
                }
            }
        ...
        private ConcurrentDictionary<string, HttpControllerDescriptor> GetControllerTypes()
        {
            var result = new ConcurrentDictionary<string, HttpControllerDescriptor>(base.GetControllerMapping());
